    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddRectFilled(title_bar_rect.GetTL(), title_bar_rect.GetBR(), GetColorU32(ImColor(0,0,0,0)), window_rounding, ImGuiCorner_TopLeft|ImGuiCorner_TopRight);
...
bool ImGui::GameEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), ImColor(23, 23, 23, 210), true, 4);


    if (active || hovered) {
        RenderFrame(bb.Min, bb.Min + ImVec2(3, 28), ImColor(0, 119, 224, 210), true, 0);

        window->DrawList->AddText(ImVec2(bb.Min.x + 9, bb.Min.y + 5 ), ImColor(225, 225, 225,255), label);
    }
    else {
        window->DrawList->AddText(ImVec2(bb.Min.x + 9, bb.Min.y + 5), ImColor(225, 225, 225, 255), label);
    }
...
bool ImGui::Game(const char* label, const bool active, const ImVec2& size_arg)
{
    return GameEx(label, active, size_arg, 0);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, ImColor(0, 119, 224), false, 4);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
    ...
    // Render
    const ImU32 col = GetColorU32(ImColor(50,50,50));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRect(check_bb.Min + ImVec2(3,3), check_bb.Max - ImVec2(3, 3), ImColor(0, 119, 224), 4, 15, 2);
    if (*v)
    {
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(3, 3), check_bb.Max - ImVec2(3, 3), ImColor(0, 119, 224), 4, 15);
    }
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (!is_multiline)
    {
        RenderFrame(frame_bb.Min - ImVec2(2, 2), frame_bb.Max + ImVec2(2, 2), ImColor(120, 120, 120, 60), false, 3);

        if (hovered)
            window->DrawList->AddRect(frame_bb.Min - ImVec2(2, 2), frame_bb.Max + ImVec2(2, 2), ImColor(0, 119, 224, 90), 3);
        else
            window->DrawList->AddRect(frame_bb.Min - ImVec2(2, 2), frame_bb.Max + ImVec2(2, 2), ImColor(140, 140, 140, 90), 3);
    }
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos - render_scroll, ImColor(41, 116, 254, 255), buf_display, buf_display + edit_state.CurLenA, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(),ImColor(230, 230, 230, 140));
    ...
    else
    {
        ...
        draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos + ImVec2(5,0), ImColor(230, 230, 230, 140), buf_display, buf_end, 0.0f, is_multiline ? NULL : &clip_rect);
    }
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 20 - style.FramePadding.y), label);
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
    ...
    if (label_size.x > 0)
    {
        ...
        ItemSize(label_size + ImVec2(0,2), style.FramePadding.y);
    }
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    if (*current_item >= 0 && *current_item < items_count)
    {
        ...
        if (items_getter(data, *current_item, &item_text))
            RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(5,0), value_bb.Max, item_text, NULL, NULL, ImVec2(0.0f,0.0f));
    }

    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + 5 +  style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
